...
BEGIN
  ...
  WITH hours AS (
    ...
    SELECT COALESCE(h.employee_id, l.employee_id) AS employee_id,
           COALESCE(h.wd, 0) + COALESCE(l.wd, 0)  AS wd
    FROM   hours h FULL JOIN hol l ON l.employee_id = h.employee_id
  )
