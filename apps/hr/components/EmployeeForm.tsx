...
const EmployeeForm: React.FC<Props> = ({
  ...
  return (
    <div className="animate-fadeInUp space-y-8">
        ...
        <div className={sectionCls}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="flex flex-col justify-center">
              <label className={labelCls}>Bảng lương</label>
              <label className="flex items-center gap-2 cursor-pointer mt-1">
                <input
                  type="checkbox"
                  className="w-4 h-4 accent-orange-500"
                  checked={!!(form as any).exclude_from_payroll}
                  onChange={e => setForm(f => ({ ...f, exclude_from_payroll: e.target.checked }))}
                />
                <span className="text-xs text-neutral-medium">Không tính lương tự động</span>
              </label>
              {(form as any).exclude_from_payroll && (
      ...
      {/* ── Modal: Cập nhật lương khi lên chính thức ── */}
      {showOfficialSalaryModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => !savingOfficialSalary && setShowOfficialSalaryModal(false)}>
          {/* Backdrop */}
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" />
          {/* Modal */}
          <div
            className="relative w-full max-w-lg rounded-[20px] border border-white/[0.08] p-6 space-y-5 animate-scaleIn shadow-2xl"
            style={{ background: '#1A1A1A' }}
            onClick={e => e.stopPropagation()}
          >
            <div>
              <h3 className="text-base font-black uppercase tracking-wider text-white">Cập nhật lương khi lên chính thức?</h3>
              <p className="text-xs text-neutral-medium mt-1">
                Ngày chính thức: <span className="text-orange-400 font-bold">{pendingOfficialDate}</span>
              </p>
              <p className="text-[10px] text-neutral-medium/60 mt-1">
                Điều chỉnh mức lương mới cho giai đoạn chính thức. Bản ghi lương thử việc sẽ được giữ nguyên.
              </p>
            </div>
            ...
            <div className="space-y-2 max-h-[320px] overflow-y-auto pr-1">
              {salaryComponents.map(comp => (
                <div key={comp.id} className="flex items-center gap-3 p-3 rounded-xl border border-white/[0.04]" style={{ background: 'rgba(255,255,255,0.02)' }}>
                  <span className="flex-1 text-white font-bold text-sm truncate">{comp.name}</span>
                  <div className="flex items-center gap-2">
                    <input
                      type="number"
                      className="w-36 px-3 py-2 rounded-lg border border-white/10 text-white text-sm text-right outline-none focus:border-orange-500/50 transition-colors"
                      style={{ background: '#1a1a1a' }}
                      value={officialSalaryDraft[comp.id] || ''}
                      onChange={e => setOfficialSalaryDraft(prev => ({ ...prev, [comp.id]: +e.target.value }))}
                      placeholder="0"
                    />
                    <span className="text-neutral-medium text-xs w-8">VNĐ</span>
                  </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-white/[0.06]">
              <span className="text-[10px] font-black uppercase tracking-widest text-orange-400">Tổng gross mới</span>
              <div>
                <span className="text-white font-black text-lg">{(Object.values(officialSalaryDraft) as number[]).reduce((s, v) => s + (v || 0), 0).toLocaleString()}</span>
                <span className="text-neutral-medium text-xs ml-1">VNĐ</span>
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-1">
              <button
                type="button"
                disabled={savingOfficialSalary}
                onClick={() => { setShowOfficialSalaryModal(false); onCancel(); }}
                className="flex-1 px-4 py-2 rounded-xl text-xs font-black uppercase text-neutral-400 border border-white/10 hover:bg-white/5 transition-all disabled:opacity-30"
              >
                Bỏ qua
              </button>
              <button
                ...
                disabled={savingOfficialSalary}
                onClick={async () => {
                  if (!editingEmployee || !pendingOfficialDate) return;
                  setSavingOfficialSalary(true);
                  try {
                    const oldTotal = (Object.values(salaryAmounts) as number[]).reduce((s, v) => s + (v || 0), 0);
                    const newTotal = (Object.values(officialSalaryDraft) as number[]).reduce((s, v) => s + (v || 0), 0);

                    // Save new salary records (effective_from = official_date)
                    for (const comp of salaryComponents) {
                      const amt = officialSalaryDraft[comp.id] || 0;
                      if (amt > 0) {
                        await svc.saveEmployeeSalary({
                          employee_id: editingEmployee.id,
                          ...
                          effective_from: pendingOfficialDate,
                          effective_to: null,
                        });
                      }
                    }

                    // Record salary change in position history
                    await svc.addPositionChange({
                      employee_id: editingEmployee.id,
                      change_type: 'salary',
                      old_value: oldTotal.toLocaleString() + ' VNĐ',
                      new_value: newTotal.toLocaleString() + ' VNĐ',
                      effective_date: pendingOfficialDate,
                      reason: 'Cập nhật lương khi lên chính thức',
                    });

                    setShowOfficialSalaryModal(false);
                    onCancel(); // Navigate back to employee list
                  } catch (err: any) {
                    alert('Lỗi: ' + (err.message || 'Không thể lưu lương'));
                  } finally {
                    setSavingOfficialSalary(false);
                  }
                }}
