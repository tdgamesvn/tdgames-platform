...
export const Navbar: React.FC<NavbarProps> = ({ theme, currentUser, activeTab, accessibleTabs, onTabChange, onLogout, vcbRate, vcbRateLoading, onBack, appName, tabLabels, onHelp }) => (
  <nav className={`h-14 md:h-20 sticky top-0 backdrop-blur-md border-b flex items-center justify-between px-3 md:px-12 z-50 transition-colors duration-300 ${theme === 'dark' ? 'bg-[#0F0F0F]/95 border-primary/10' : 'bg-white/95 border-gray-200 shadow-sm'}`}>
      ...
      <div className={`flex gap-1 p-1 rounded-full border overflow-x-auto scrollbar-hide ${theme === 'dark' ? 'bg-surface border-white/5' : 'bg-gray-100 border-gray-200'}`}>
        {accessibleTabs.map((tab) => (
          <button
            ...
            className={`flex-shrink-0 px-3 md:px-6 py-1.5 md:py-2 rounded-full text-[9px] md:text-xs font-black uppercase tracking-wider md:tracking-widest transition-all whitespace-nowrap ${activeTab === tab ? 'bg-primary text-black shadow-btn-glow' : theme === 'dark' ? 'text-neutral-medium hover:text-white' : 'text-gray-500 hover:text-black'}`}
          >
             {tabLabels?.[tab] || tab}
          </button>
        ))}
